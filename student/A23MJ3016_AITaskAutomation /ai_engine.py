...
class AIEngine:
    ...
    def _generate_initial_state(self) -> SystemState:
        ...
        return SystemState(
            tasks=[
                Task(id="task-1", name="Quality Inspection A", status="pending", priority="high"),
                Task(id="task-2", name="Assembly Line Setup", status="pending", priority="medium"),
                Task(id="task-3", name="Packaging Process", status="pending", priority="medium"),
                Task(id="task-4", name="Inventory Check", status="pending", priority="low"),
                Task(id="task-5", name="Conveyor System Check", status="pending", priority="high"), 
            ],
            workers=[
                Worker(id="worker-1", name="Robot Arm A", status="available", skill_type="assembly"),
                Worker(id="worker-2", name="Inspector Unit B", status="available", skill_type="inspection"),
                Worker(id="worker-3", name="Packaging Bot C", status="available", skill_type="packaging"),
            ],
            machines=[
                Machine(id="machine-1", name="CNC Machine 01", temperature=55.0, vibration=0.7, status="normal"),
                Machine(id="machine-2", name="Press Unit 02", temperature=62.0, vibration=0.8, status="normal"),
                Machine(id="machine-3", name="Conveyor System 03", temperature=48.0, vibration=0.5, status="normal"),
            ],
            resources=[
                Resource(id="resource-1", name="Power Consumption", current_usage=85.0, optimal_usage=75.0, is_optimized=False),
                Resource(id="resource-2", name="Material Flow", current_usage=92.0, optimal_usage=80.0, is_optimized=False),
                Resource(id="resource-3", name="Network Bandwidth", current_usage=68.0, optimal_usage=70.0, is_optimized=True),
            ],
            alerts=[],
            action_history=[],
            current_step=0
